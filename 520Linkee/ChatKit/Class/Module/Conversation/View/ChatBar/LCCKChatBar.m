...
//  LCCKChatBarExample
//
//  v0.7.10 Created by ElonChan (微信向我报BUG:chenyilong1010) ( https://github.com/leancloud/ChatKit-OC ) on 15/8/17.
...
- (void)setupConstraints {
    ...
    [self.messageTypeSwitch mas_makeConstraints:^(MASConstraintMaker *make) {
...
//    }];
    ...
    [self.textView mas_makeConstraints:^(MASConstraintMaker *make) {
        make.left.equalTo(self.messageTypeSwitch.mas_right).with.offset(offset);
...
- (void)setup {
    self.close = NO;
    self.messageTypeSwitch.selected = NO;
...
//    [self.inputBarBackgroundView addSubview:self.voiceButton];
    [self.inputBarBackgroundView addSubview:self.messageTypeSwitch];
...
/**
 *  lazy load messageTypeSwitch
 *
 *  @return UISwitch
 */
- (UISwitch *)messageTypeSwitch {
    if (_messageTypeSwitch == nil) {
        UISwitch *messageTypeSwitch = [[UISwitch alloc] init];
        [messageTypeSwitch addTarget:self action:@selector(didSwitchMessageType:) forControlEvents:UIControlEventTouchUpInside];
        [messageTypeSwitch sizeToFit];
        _messageTypeSwitch = messageTypeSwitch;
    }
    return _messageTypeSwitch;
}

- (void)didSwitchMessageType:(UISwitch *)sender {
    sender.selected = !sender.selected;
    [LCCKConversationService sharedInstance].danMuMessage = sender.selected;
}
